...
/* ── Re-scan GPS: clear negative cache + route cache and re-fetch ── */
export async function hmRescanGPS() {
  try {
    const db = await _hmOpenDB();
    // Clear noGps blacklist
    const tx1 = db.transaction(HM_META_STORE, 'readwrite');
    tx1.objectStore(HM_META_STORE).delete('noGpsIds');
    await new Promise((r, j) => { tx1.oncomplete = r; tx1.onerror = j; });
    // Clear route cache
    const tx2 = db.transaction(HM_STORE, 'readwrite');
    tx2.objectStore(HM_STORE).clear();
    await new Promise((r, j) => { tx2.oncomplete = r; tx2.onerror = j; });
    // Clear bg timestamp
    const tx3 = db.transaction(HM_META_STORE, 'readwrite');
    tx3.objectStore(HM_META_STORE).delete('lastBgUpdate');
    await new Promise((r, j) => { tx3.oncomplete = r; tx3.onerror = j; });
    db.close();
  } catch (_) {}

  // Clear localStorage GPS caches too
...
export async function _hmBackgroundUpdate(cachedRoutes) {
  ...
  for (let i = 0; i < newActs.length; i += BATCH) {
    ...
    results.forEach((r, idx) => {
      if (r && r.id)         newRoutes.push(r);
      else if (r && r.noGps) failedIds.push(batch[idx].id);
      // transient errors: don't blacklist — will retry on next bg update
    });
    if (i + BATCH < newActs.length) await new Promise(r => setTimeout(r, 300));
  }

  // Save negative cache only for activities confirmed to have no GPS
...
// Returns: route object on success, { noGps: true } if definitively no GPS,
//          { transient: true } on recoverable errors (rate limit, timeout, network)
export async function _hmFetchOneRoute(a) {
  ...
  if (!points || points.length < 2) {
    let latlng;
    try {
      latlng = await fetchMapGPS(a.id);
    } catch (err) {
      // Rate limit or transient network error — don't blacklist
...
export async function _hmFetchAllRoutes(subEl, textEl) {
  ...
  const routes = [];
  const noGpsIds = [];
  const transientFails = []; // activities that failed due to network/rate-limit
  let loaded = 0;
  let foundGPS = 0;
  ...
  for (let i = 0; i < candidates.length; i += BATCH) {
    ...
    if (hitRateLimit) {
      if (subEl) subEl.textContent = `${loaded} of ${candidates.length} checked · ${foundGPS} routes · pausing for rate limit…`;
    ...
    const results = await Promise.all(batch.map(a => _hmFetchOneRoute(a)));
    results.forEach((r, idx) => {
      if (r && r.noGps)          noGpsIds.push(batch[idx].id);
      else if (r && r.transient) { transientFails.push(batch[idx]); if (r.rateLimited) hitRateLimit = true; }
      else if (r && r.id)        { routes.push(r); foundGPS++; }
      else                       noGpsIds.push(batch[idx].id); // shouldn't happen, but treat as no GPS
    });
    loaded += batch.length;
  ...
  // Retry pass for transient failures (network/timeout/rate-limit)
  if (transientFails.length > 0) {
    if (subEl) subEl.textContent = `Retrying ${transientFails.length} failed routes…`;
    await new Promise(r => setTimeout(r, 2000)); // longer pause before retry

    const RETRY_BATCH = 2;
    for (let i = 0; i < transientFails.length; i += RETRY_BATCH) {
      const batch = transientFails.slice(i, i + RETRY_BATCH);
      const results = await Promise.all(batch.map(a => _hmFetchOneRoute(a)));
      results.forEach((r, idx) => {
        if (r && r.id)      { routes.push(r); foundGPS++; }
        else if (r && r.noGps) noGpsIds.push(batch[idx].id);
        // Still transient on retry — don't blacklist, just skip this scan
      });
      if (subEl) subEl.textContent = `Retrying… ${foundGPS} routes found`;
      if (i + RETRY_BATCH < transientFails.length) await new Promise(r => setTimeout(r, 1000));
    }
  }
...
export function hmUpdateStats(routes) {
  ...
  if (el('hmStatRoutes')) {
    const total = _hm.allRoutes ? _hm.allRoutes.length : routes.length;
    el('hmStatRoutes').textContent = routes.length === total ? routes.length : `${routes.length} / ${total}`;
  }
  if (el('hmStatRoutesLabel')) {
    const totalActs = getAllActivities().length;
    el('hmStatRoutesLabel').textContent = `of ${totalActs.toLocaleString()} activities`;
  }
