...
export async function stravaExchangeCode(code) {
  ...
  try {
    ...
    navigate('settings');
    setTimeout(() => stravaRenderSyncUI(), 200);
  } catch (e) {
...
export function icuRenderSyncUI() {
  ...
  if (connected && state.athlete) {
    ...
    if (badgeEl) { badgeEl.textContent = 'Connected'; badgeEl.className = 'connection-status-badge connected'; }
    ...
    if (lastSyncEl) lastSyncEl.textContent = syncLabel;
    const headerSyncEl = document.getElementById('icuLastSync');
    if (headerSyncEl) headerSyncEl.textContent = ts ? `Synced ${syncLabel.toLowerCase()}` : syncLabel;
    ...
    if (state.currentPage === 'import' || state.currentPage === 'settings') {
      updateStorageBar();
