...
const _removeCyclOSMLayer  = _app('_removeCyclOSMLayer');
const _addRoadSafetyLayer  = _app('_addRoadSafetyLayer');
const _removeRoadSafetyLayer = _app('_removeRoadSafetyLayer');
const setRoadSafetyEnabled = _app('setRoadSafetyEnabled');
const setCyclOSMEnabled    = _app('setCyclOSMEnabled');
...
const setTerrainEnabled    = _app('setTerrainEnabled');
...
const _rb = {
  ...
  _snapToRoads: localStorage.getItem('rb_snap') !== 'false',
  _avoidUnpaved: localStorage.getItem('rb_avoid_unpaved') === 'true',
  _avoidHighways: localStorage.getItem('rb_avoid_highways') === 'true',
  _gradientMode: null, // null | 'slope' | 'elevation'
  _windOverlay: false,
  _windData: null,
  _windMarker: null,
  _elevShading: false,
          ...
          <div class="rb-options-wrap" id="rbOptionsWrap">
            ...
            <div class="rb-options-menu" id="rbOptionsMenu">
              ...
              <label class="rb-options-toggle">
                ...
                <input type="checkbox" id="rbOptSnap" ${_rb._snapToRoads ? 'checked' : ''} onchange="rbToggleSnap(this.checked)" />
                <div class="rb-toggle-track"><div class="rb-toggle-thumb"></div></div>
              </label>
              <label class="rb-options-toggle">
                <span>Avoid unpaved</span>
                <input type="checkbox" id="rbOptUnpaved" ${_rb._avoidUnpaved ? 'checked' : ''} onchange="rbToggleAvoidUnpaved(this.checked)" />
                <div class="rb-toggle-track"><div class="rb-toggle-thumb"></div></div>
              </label>
              <label class="rb-options-toggle">
                <span>Avoid highways</span>
                <input type="checkbox" id="rbOptHighways" ${_rb._avoidHighways ? 'checked' : ''} onchange="rbToggleAvoidHighways(this.checked)" />
                <div class="rb-toggle-track"><div class="rb-toggle-thumb"></div></div>
              </label>
...
export function _rbInitMap() {
  ...
  // ── Fancy Apple Maps-style compass (bottom-left, above elev panel) ──
  {
    ...
    for (let i = 0; i < 72; i++) {
      ...
      const r1 = isMajor ? 17.5 : isMid ? 18.5 : 19.5;
      const r2 = 21;
      const rad = angle * Math.PI / 180;
      const x1 = 24 + r1 * Math.sin(rad);
      const y1 = 24 - r1 * Math.cos(rad);
      const x2 = 24 + r2 * Math.sin(rad);
      const y2 = 24 - r2 * Math.cos(rad);
      const sw = isMajor ? 1.2 : isMid ? 0.8 : 0.5;
      const op = isMajor ? 0.9 : isMid ? 0.6 : 0.3;
      ticks.push(`<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="rgba(255,255,255,${op})" stroke-width="${sw}"/>`);
    }
    const el = document.createElement('div');
    el.className = 'rb-compass';
    el.id = 'rbCompass';
    el.title = 'Reset bearing to north';
    el.innerHTML = `<svg viewBox="0 0 48 48" width="48" height="48">
    ...
    el.addEventListener('click', () => _rb.map.resetNorthPitch());
    const svg = el.querySelector('svg');
    _rb.map.on('rotate', () => { svg.style.transform = `rotate(${-_rb.map.getBearing()}deg)`; });
    document.querySelector('.rb-map-container').appendChild(el);
  }
...
export function _rbInitRouterDropdown() {
  ...
  trigger.addEventListener('click', (e) => {
    ...
    const ow = document.getElementById('rbOptionsWrap');
    if (ow) ow.classList.remove('rb-options-wrap--open');
  });
...
/* ── Options Dropdown ── */
export function _rbInitOptionsDropdown() {
  const wrap = document.getElementById('rbOptionsWrap');
  const trigger = document.getElementById('rbOptionsTrigger');
  if (!wrap || !trigger) return;

  trigger.addEventListener('click', (e) => {
    e.stopPropagation();
    wrap.classList.toggle('rb-options-wrap--open');
  ...
  // Keep menu open on clicks inside it (toggles)
  wrap.querySelector('.rb-options-menu').addEventListener('click', (e) => {
    e.stopPropagation();
  });

  document.addEventListener('click', () => {
    wrap.classList.remove('rb-options-wrap--open');
  });
}

/* ── Options: Snap to Roads ── */
export function rbToggleSnap(on) {
  _rb._snapToRoads = on;
  try { localStorage.setItem('rb_snap', on ? 'true' : 'false'); } catch(_){}
  showToast(on ? 'Snap to roads on' : 'Snap to roads off — straight lines', 'info');
  if (_rb.waypoints.length >= 2) _rbRefetchAllSegments();
}

/* ── Options: Avoid Unpaved ── */
export function rbToggleAvoidUnpaved(on) {
  _rb._avoidUnpaved = on;
  try { localStorage.setItem('rb_avoid_unpaved', on ? 'true' : 'false'); } catch(_){}
  showToast(on ? 'Avoiding unpaved roads' : 'Unpaved roads allowed', 'info');
  if (_rb.waypoints.length >= 2) _rbRefetchAllSegments();
}

/* ── Options: Avoid Highways ── */
export function rbToggleAvoidHighways(on) {
  _rb._avoidHighways = on;
  try { localStorage.setItem('rb_avoid_highways', on ? 'true' : 'false'); } catch(_){}
  showToast(on ? 'Avoiding highways' : 'Highways allowed', 'info');
  if (_rb.waypoints.length >= 2) _rbRefetchAllSegments();
}

/* ── Options: Gradient Coloring ── */
export function rbSetGradient(mode) {
  // Mutually exclusive with each other and elevation shading
  _rb._gradientMode = mode;
  if (mode) { _rb._elevShading = false; const es = document.getElementById('rbOptElevShade'); if (es) es.checked = false; }
  // Uncheck other gradient checkbox
  if (mode === 'slope') { const el = document.getElementById('rbOptGradElev'); if (el) el.checked = false; }
  if (mode === 'elevation') { const el = document.getElementById('rbOptGradSlope'); if (el) el.checked = false; }
  if (!mode) {
    const s = document.getElementById('rbOptGradSlope'); if (s) s.checked = false;
    const e = document.getElementById('rbOptGradElev'); if (e) e.checked = false;
  }
  _rbRedrawRoute();
  _rbUpdateGradientLegend();
  showToast(mode ? `Gradient: ${mode}` : 'Gradient off', 'info');
}

/* ── Options: Elevation Shading ── */
export function rbToggleElevShading(on) {
  _rb._elevShading = on;
  if (on) { _rb._gradientMode = null; const s = document.getElementById('rbOptGradSlope'); if (s) s.checked = false; const e = document.getElementById('rbOptGradElev'); if (e) e.checked = false; }
  _rbRedrawRoute();
  _rbUpdateGradientLegend();
  showToast(on ? 'Elevation shading on' : 'Elevation shading off', 'info');
}

/* ── Options: Wind Overlay ── */
export function rbToggleWind(on) {
  _rb._windOverlay = on;
  if (on) { _rbFetchWind(); }
  else { _rbRemoveWindMarker(); }
  showToast(on ? 'Wind overlay on' : 'Wind overlay off', 'info');
}

/* ── Wind fetch from Open-Meteo ── */
async function _rbFetchWind() {
  if (!_rb.map || _rb.routeSegments.length === 0) {
    // Use map center if no route
    const c = _rb.map ? _rb.map.getCenter() : null;
    if (!c) return;
    return _rbFetchWindAt(c.lat, c.lng);
  }
  // Route midpoint
  const allPts = [];
  ...
  return _rbFetchWindAt(mid[0], mid[1]);
}

async function _rbFetchWindAt(lat, lng) {
  try {
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat.toFixed(4)}&longitude=${lng.toFixed(4)}&current=wind_speed_10m,wind_direction_10m,wind_gusts_10m&wind_speed_unit=ms&timezone=auto`;
    ...
    console.warn('[RB] Wind fetch failed:', e);
  }
}

function _rbRenderWindMarker() {
  _rbRemoveWindMarker();
  if (!_rb._windData || !_rb.map) return;
  const d = _rb._windData;
  const el = document.createElement('div');
  el.className = 'rb-wind-marker';
  const dirs = ['N','NE','E','SE','S','SW','W','NW'];
  const dirLabel = dirs[Math.round(d.direction / 45) % 8];
  el.innerHTML = `
    <div class="rb-wind-arrow" style="transform: rotate(${d.direction}deg)">
    ...
    <div class="rb-wind-label">${d.speed.toFixed(1)} m/s ${dirLabel}</div>
    ${d.gusts ? `<div class="rb-wind-gusts">Gusts ${d.gusts.toFixed(1)} m/s</div>` : ''}
  `;
  _rb._windMarker = new maplibregl.Marker({ element: el, anchor: 'center' })
    .setLngLat([d.lng, d.lat]).addTo(_rb.map);
}
...
function _rbUpdateGradientLegend() {
  let legend = document.getElementById('rbGradientLegend');
  const active = _rb._gradientMode || _rb._elevShading;
  if (!active) { if (legend) legend.style.display = 'none'; return; }
  if (!legend) {
    legend = document.createElement('div');
    legend.id = 'rbGradientLegend';
    legend.className = 'rb-gradient-legend';
    const mc = document.querySelector('.rb-map-container');
    if (mc) mc.appendChild(legend);
  }
  legend.style.display = '';
  if (_rb._gradientMode === 'slope') {
    legend.innerHTML = `
    ...
    `;
  } else if (_rb._gradientMode === 'elevation' || _rb._elevShading) {
    legend.innerHTML = `
...
export function _rbCycleMapLayer() {
  if (!_rb.map) return;
  _rbEnsureLayerKeys();
  // Sync from window in case setMapTheme() updated it externally
  if (typeof window._rbLayerIdx === 'number') _rbLayerIdx = window._rbLayerIdx;
    ...
    try {
      ...
      if (resp.ok) {
        ...
        if (feat) {
          ...
          // Extract surface tags from BRouter messages
          const surfaces = _parseBrouterSurfaces(feat);
...
/* ── BRouter surface parsing ── */
const _UNPAVED_HIGHWAYS = new Set(['track', 'path', 'bridleway', 'steps']);
const _PAVED_HIGHWAYS = new Set(['motorway', 'trunk', 'primary', 'secondary', 'tertiary', 'residential', 'unclassified', 'service', 'living_street', 'motorway_link', 'trunk_link', 'primary_link', 'secondary_link', 'tertiary_link']);

function _parseBrouterSurfaces(feat) {
  ...
  for (let i = 1; i < msgs.length; i++) {
    const tags = msgs[i][tagIdx] || '';
    const sm = tags.match(/surface=(\S+)/);
    const hm = tags.match(/highway=(\S+)/);
    const segDist = distIdx >= 0 ? parseFloat(msgs[i][distIdx]) || 0 : 0;
    let surface;
    if (sm) {
      surface = sm[1];
    } else if (hm) {
      // Infer surface from highway type when surface tag is missing
      if (_UNPAVED_HIGHWAYS.has(hm[1])) surface = '_inferred_unpaved';
      else if (_PAVED_HIGHWAYS.has(hm[1])) surface = '_inferred_paved';
      else surface = 'unknown';
    } else {
      surface = 'unknown';
    }
    surfaces.push({ surface, distance: segDist });
  }
  return surfaces;
}
...
export async function _brouterFetchRoute(from, to, altIdx, signal) {
  let profile = _rb.router.profile;
  // Override profile for avoid preferences (only if using a standard road profile)
  if (_rb._avoidUnpaved && ['fastbike', 'trekking'].includes(profile)) profile = 'safety';
  if (_rb._avoidHighways && ['fastbike'].includes(profile)) profile = 'fastbike-lowtraffic';
  const lonlats = `${from.lng},${from.lat}|${to.lng},${to.lat}`;
...
function _rbRoutingShow() {
  _rb._routingCount++;
  const el = document.getElementById('rbRoutingStatus');
  if (el) el.classList.add('rb-routing-status--active');
}
...
export async function _rbFetchRoute(from, to, withAlternatives, skipAbort) {
  ...
  if (!_rb._snapToRoads) {
    ...
    const dist = _rbHaversine(pts[0], pts[1]);
    const result = { points: pts, distance: dist, duration: 0, annotations: null, fallback: true };
...
export function _rbRedrawRoute() {
  ...
  try { if (_rb.map.getSource('rb-gradient')) _rb.map.removeSource('rb-gradient'); } catch(_){}
...
/* ── Climb Detection ── */
const _CLIMB_CAT_COLORS = { 'HC': '#e53935', '1': '#ff5722', '2': '#ff9800', '3': '#ffc107', '4': '#8bc34a', 'minor': '#78909c' };

function _rbDetectClimbs() {
  const ed = _rb.elevationData;
  if (!ed || ed.length < 5) return [];

  // Smooth elevation to reduce noise (moving avg of 5 points)
  const smooth = ed.map((p, i) => {
    const start = Math.max(0, i - 2), end = Math.min(ed.length - 1, i + 2);
    let sum = 0, cnt = 0;
    for (let j = start; j <= end; j++) { sum += ed[j].elev; cnt++; }
    return { ...p, elev: sum / cnt };
  });

  // Detect climbs: rising segments with gain
  const climbs = [];
  let climbStart = null;
  let maxElev = smooth[0].elev;
  let minElev = smooth[0].elev;
  let localMin = 0;

  for (let i = 1; i < smooth.length; i++) {
    const e = smooth[i].elev;
    if (climbStart === null) {
      // Looking for a climb start
      if (e > minElev + 5) {
        climbStart = localMin;
        maxElev = e;
      }
      if (e < minElev) { minElev = e; localMin = i; }
    } else {
      // In a climb
      if (e > maxElev) maxElev = e;
      // Climb ends if we drop >15m from max or >10% of gained elevation (min 20m)
      const gained = maxElev - smooth[climbStart].elev;
      const dropped = maxElev - e;
      if (dropped > 15 || (gained > 30 && dropped > gained * 0.1) || i === smooth.length - 1) {
        // Find the actual peak index
        let peakIdx = climbStart;
        for (let j = climbStart; j <= i; j++) {
          if (smooth[j].elev > smooth[peakIdx].elev) peakIdx = j;
        }
        const gain = smooth[peakIdx].elev - smooth[climbStart].elev;
        const dist = ed[peakIdx].dist - ed[climbStart].dist;
        if (gain >= 10 && dist > 50) {
          const avgGrade = dist > 0 ? (gain / dist) * 100 : 0;
          // Difficulty score: gain_m × avg_gradient_%
          // Validated against Tour de France: Alpe d'Huez=8461(HC), Ventoux=12075(HC)
          const score = gain * avgGrade;
          let cat;
          if (score >= 8000 || gain >= 1500) cat = 'HC';
          else if (score >= 3500 || gain >= 1000) cat = '1';
          else if (score >= 1500) cat = '2';
          else if (score >= 600) cat = '3';
          else if (score >= 200) cat = '4';
          else cat = 'minor';
          ...
          if (cat) {
            climbs.push({
              ...
              startDist: Math.round(ed[climbStart].dist),
              startIdx: climbStart,
              peakIdx: peakIdx,
              startElev: Math.round(smooth[climbStart].elev),
              peakElev: Math.round(smooth[peakIdx].elev),
              lat: ed[peakIdx].lat,
              lng: ed[peakIdx].lng,
            });
          }
...
function _rbUpdateClimbs() {
  const row = document.getElementById('rbClimbsRow');
  const list = document.getElementById('rbClimbsList');
  if (!row || !list) return;

  const climbs = _rbDetectClimbs();
  if (climbs.length === 0) {
  ...
  row.style.display = '';
  list.innerHTML = climbs.map(c =>
    `<div class="rb-climb-badge" title="${c.gain}m over ${(c.dist/1000).toFixed(1)}km at ${c.avgGrade}%">` +
    `<span class="rb-climb-cat${c.cat === 'minor' ? ' rb-climb-cat--minor' : ''}" style="background:${_CLIMB_CAT_COLORS[c.cat]}">${c.cat === 'HC' ? 'HC' : c.cat === 'minor' ? '—' : 'Cat ' + c.cat}</span>` +
    `<span class="rb-climb-info">${c.gain}m · ${c.avgGrade}%</span>` +
    `</div>`
  ).join('');
}
...
const _PAVED_SURFACES = new Set(['asphalt', 'concrete', 'paving_stones', 'sett', 'concrete:plates', 'concrete:lanes', 'paved', 'metal', '_inferred_paved']);
const _MIXED_SURFACES = new Set(['compacted', 'fine_gravel', 'cobblestone', 'unhewn_cobblestone', 'bricks']);
const _OFFROAD_SURFACES = new Set(['gravel', 'dirt', 'grass', 'ground', 'sand', 'mud', 'earth', 'wood', 'unpaved', '_inferred_unpaved']);

function _classifySurface(s) {
  if (!s || s === 'unknown') return null;
  if (_PAVED_SURFACES.has(s)) return 'paved';
  if (_MIXED_SURFACES.has(s)) return 'mixed';
  if (_OFFROAD_SURFACES.has(s)) return 'offroad';
  return 'offroad'; // anything else not recognized
}

export function _rbDetectSurfaces() {
  let paved = 0, mixed = 0, offroad = 0;
  let hasBrouterData = false;

  for (const seg of _rb.routeSegments) {
    // Prefer BRouter surface tags (actual OSM data)
    if (seg.brouterSurfaces && seg.brouterSurfaces.length > 0) {
      hasBrouterData = true;
      for (const s of seg.brouterSurfaces) {
        const cls = _classifySurface(s.surface);
        const weight = s.distance || 1;
        if (cls === 'paved') paved += weight;
        else if (cls === 'mixed') mixed += weight;
        else if (cls === 'offroad') offroad += weight;
      }
      continue;
    }
    // Fallback: OSRM speed-based heuristic
  ...
  if (hasBrouterData) {
    // Show dominant surface type with percentage
    const pp = Math.round(paved / total * 100);
    const mp = Math.round(mixed / total * 100);
    const op = Math.round(offroad / total * 100);
    if (pp >= 90) return 'Paved';
    if (op >= 90) return 'Off-road';
    if (mp >= 50) return 'Mixed';
    return pp >= op ? `${pp}% Paved` : `${op}% Unpaved`;
  }
...
export function _rbGroupBySurface() {
  const groups = [];
  for (const seg of _rb.routeSegments) {
    // Prefer BRouter surface tags (real OSM data)
    if (seg.brouterSurfaces && seg.brouterSurfaces.length > 0) {
      // BRouter messages map to coordinate points; distribute across points
      const pts = seg.points;
      const surfs = seg.brouterSurfaces;
      // Each message corresponds roughly to a waypoint — group consecutive same-surface points
      const ptsPerMsg = Math.max(1, Math.floor(pts.length / surfs.length));
      let curType = _classifySurface(surfs[0].surface) || 'paved';
      let curPts = [pts[0]];
      for (let i = 1; i < pts.length; i++) {
        const msgIdx = Math.min(Math.floor(i / ptsPerMsg), surfs.length - 1);
        const t = _classifySurface(surfs[msgIdx].surface) || 'paved';
        if (t !== curType) {
          curPts.push(pts[i]);
...
/* ── Gradient / Elevation Shading segments ── */
function _rbBuildGradientSegments(routedPoints) {
  const ed = _rb.elevationData;
  if (!ed || ed.length < 2 || routedPoints.length < 2) return [];

  const features = [];
  const mode = _rb._gradientMode || (_rb._elevShading ? 'elevation' : null);
  if (!mode) return [];

  // Build value range for elevation shading
  let minElev = Infinity, maxElev = -Infinity;
  if (mode === 'elevation') {
    for (const d of ed) { if (d.elev < minElev) minElev = d.elev; if (d.elev > maxElev) maxElev = d.elev; }
  }
  const elevRange = maxElev - minElev || 1;

  // Walk through elevation data points and assign colors to segments
  for (let i = 0; i < ed.length - 1; i++) {
    let color;
    if (mode === 'slope') {
      const ag = Math.abs(ed[i].grade);
      color = ag < 3 ? '#00e5a0' : ag < 6 ? '#f0c429' : ag < 10 ? '#ff6b35' : '#ff4757';
    } else {
      // elevation — normalize to 0–1
      const t = (ed[i].elev - minElev) / elevRange;
      color = t < 0.25 ? '#00e5a0' : t < 0.5 ? '#f0c429' : t < 0.75 ? '#ff6b35' : '#ff4757';
    }
    features.push({
      type: 'Feature',
      properties: { color },
      geometry: {
        type: 'LineString',
        coordinates: [[ed[i].lng, ed[i].lat], [ed[i + 1].lng, ed[i + 1].lat]],
      },
    });
  }
  return features;
...
export function _rbToggleRoadSafety() {
  if (!_rb.map) return;
  const isSat = _rbMapLayerKeys ? _rbMapLayerKeys[_rbLayerIdx] === 'satellite' : false;
...
export async function rbSave() {
  ...
  const route = {
    ...
    distance: totalDist,
    elevGain, elevLoss,
    segments: _rb.routeSegments.map(s => ({ points: s.points, distance: s.distance, duration: s.duration, annotations: s.annotations || null, brouterSurfaces: s.brouterSurfaces || null })),
  };
...
// ── Edit Mode (Cancel / Save bar for loaded routes) ──

function _rbShowEditBar(show) {
  const bar = document.getElementById('rbEditBar');
  if (!bar) return;
  if (show && _rb.activeRouteId) {
    const route = _rb.savedRoutes.find(r => r.id === _rb.activeRouteId);
    const nameEl = document.getElementById('rbEditBarName');
    if (nameEl) nameEl.textContent = route?.name || 'Editing route';
    bar.style.display = '';
  } else {
...
export function rbCancelEdit() {
  rbClear();
}

export async function rbSaveEdit() {
  if (!_rb.activeRouteId || _rb.waypoints.length < 2) return;
  ...
  const existing = _rb.savedRoutes.find(r => r.id === _rb.activeRouteId);
  const route = {
    id: _rb.activeRouteId,
    name: existing?.name || 'Edited Route',
    ts: Date.now(),
    waypoints: _rb.waypoints.map(w => ({ lat: w.lat, lng: w.lng })),
    routePoints: allPoints,
    elevationData: _rb.elevationData,
...
// Split an activity-sourced route (single mega-segment) into multiple editable segments
// by sampling intermediate waypoints at regular distance intervals along the GPS trace.
function _rbSplitActivityRoute(route) {
  const pts = route.segments?.[0]?.points;
  if (!pts || pts.length < 4) return null;

  // Build cumulative distance array from GPS points
  const cumDist = [0];
  for (let i = 1; i < pts.length; i++) {
    cumDist[i] = cumDist[i - 1] + _rbHaversine(pts[i - 1], pts[i]);
  }
  const totalDist = cumDist[pts.length - 1];
  if (totalDist < 500) return null; // too short to split

  // Pick interval: target ~5km, but adjust for short routes (min 8 segments, max 5km)
  const interval = Math.max(1000, Math.min(5000, totalDist / 12));

  // Walk through points and mark split positions
  const splitIndices = [0]; // always start at index 0
  let nextThreshold = interval;
  for (let i = 1; i < pts.length - 1; i++) {
    if (cumDist[i] >= nextThreshold) {
      splitIndices.push(i);
      nextThreshold = cumDist[i] + interval;
    }
  }
  splitIndices.push(pts.length - 1); // always end at last point

  // Need at least 3 waypoints (start + 1 intermediate + end) to be useful
  if (splitIndices.length < 3) return null;

  // Build waypoints and segments from split positions
  const waypoints = splitIndices.map(idx => ({ lat: pts[idx][0], lng: pts[idx][1] }));
  const segments = [];
  const totalDuration = route.segments[0].duration || 0;
  ...
  for (let i = 0; i < splitIndices.length - 1; i++) {
    ...
    const segDist = cumDist[endIdx] - cumDist[startIdx];
    const segDur = totalDist > 0 ? totalDuration * (segDist / totalDist) : 0;
    segments.push({
      points: segPts,
      distance: segDist,
      duration: segDur,
...
export async function rbLoadRoute(id) {
  ...
  // Activity routes: auto-split single mega-segment into editable chunks
  let loadWaypoints = route.waypoints;
  let loadSegments = route.segments || [];
  if (route.fromActivity && loadSegments.length === 1 && loadWaypoints.length <= 2) {
    const split = _rbSplitActivityRoute(route);
    if (split) {
      loadWaypoints = split.waypoints;
      loadSegments = split.segments;
    }
  }

  loadWaypoints.forEach((w, i) => {
    const marker = _rbCreateWaypointMarker(w.lat, w.lng, i);
    marker.on('dragend', () => _rbOnWaypointDrag(i));
...
export function rbClear() {
  ...
  if (_rb.map) {
    ...
    if (_rb.map.getSource('rb-gradient')) _rb.map.removeSource('rb-gradient');
  ...
  const gl = document.getElementById('rbGradientLegend');
